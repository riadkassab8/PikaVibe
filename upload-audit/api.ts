...
export type AdminCategory = { id: number; slug: string; name: string; active: boolean };
...
export async function createCategory(name: string) {
  return request<AdminCategory>('/categories', { method: 'POST', body: JSON.stringify({ name }) });
}

export async function updateCategory(id: number, name: string, active: boolean) {
  return request<AdminCategory>(`/categories/${id}`, { method: 'PUT', body: JSON.stringify({ name, active }) });
}
