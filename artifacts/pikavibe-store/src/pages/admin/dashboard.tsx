...
type ProductForm = { id?: number; name: string; nameAr: string; nameEn: string; slug: string; category: string; categoryAr: string; categoryEn: string; price: string; oldPrice: string; discountPercent: string; discountActive: boolean; discountStartsAt: string; discountEndsAt: string; installmentAvailable: boolean; installmentMinMonths: string; installmentMaxMonths: string; stock: string; description: string; descriptionAr: string; descriptionEn: string; imageUrl: string; images: string; specifications: string; specificationsAr: string; specificationsEn: string; variants: string; featured: boolean; active: boolean };
const fallbackCategories = ['Cookware', 'Storage', 'Cleaning', 'Bathroom', 'Small Appliances', 'Dining'];
const statuses = ['pending', 'confirmed', 'processing', 'shipped', 'delivered', 'cancelled'];
const emptyForm: ProductForm = { name: '', nameAr: '', nameEn: '', slug: '', category: 'Cookware', categoryAr: '', categoryEn: '', price: '', oldPrice: '', discountPercent: '', discountActive: false, discountStartsAt: '', discountEndsAt: '', installmentAvailable: false, installmentMinMonths: '2', installmentMaxMonths: '6', stock: '0', description: '', descriptionAr: '', descriptionEn: '', imageUrl: '', images: '[]', specifications: '', specificationsAr: '', specificationsEn: '', variants: '[]', featured: false, active: true };
const numberFormatter = new Intl.NumberFormat('en-US', { numberingSystem: 'latn' });
const money = (value: number) => `ج.م ${numberFormatter.format(Number(value || 0))}`;
const dashboardDate = (value: string | number | Date) => new Date(value).toLocaleDateString('en-GB', { numberingSystem: 'latn' });
...
export default function AdminDashboard() {
  ...
  return <div id="admin-dashboard-root" dir={language === 'ar' ? 'rtl' : 'ltr'} className={`min-h-screen bg-background ${language === 'ar' ? 'text-right' : 'text-left'}`}>
    ...
    <div className="mx-auto flex max-w-7xl flex-col gap-8 px-4 py-8 sm:px-6 lg:flex-row lg:px-8"><aside className="lg:w-64 lg:shrink-0"><nav className="grid grid-cols-2 gap-2 sm:grid-cols-3 lg:grid-cols-1">{tabs.map((tab) => { const Icon = tab.icon; return <button key={tab.id} onClick={() => setActiveTab(tab.id)} className={`flex items-center gap-3 rounded-xl px-4 py-3 text-sm font-semibold transition-colors ${activeTab === tab.id ? 'bg-primary text-primary-foreground shadow-soft' : 'text-foreground hover:bg-muted'}`}><Icon className="h-5 w-5" />{tab.label}</button>; })}</nav></aside>
      <main className="min-w-0 flex-1">{activeTab === 'overview' && <Overview stats={stats} orders={orders} loading={loading} onOpenOrders={() => setActiveTab('orders')} />}{activeTab === 'products' && <ProductsManagement products={products} categories={categories.length ? categories.filter((category) => category.active).map((category) => category.name) : fallbackCategories} onRefresh={refresh} />}{activeTab === 'orders' && <OrdersManagement orders={orders} loading={loading} onRefresh={refresh} />}{activeTab === 'customers' && <CustomersManagement orders={orders} loading={loading} />}{activeTab === 'categories' && <CategoryManagement categories={categories} products={products} onRefresh={refresh} />}{activeTab === 'installments' && <InstallmentsManagement />}{activeTab === 'settings' && <SettingsPanel categories={categories} products={products} onRefresh={refresh} />}</main>
    </div>
  </div>;
}
...
function toProductForm(product: ApiProduct): ProductForm {
  return { id: product.backendId, name: product.name, nameAr: product.nameAr || product.name, nameEn: product.nameEn || product.name, slug: product.id, category: product.category, categoryAr: product.categoryAr || '', categoryEn: product.categoryEn || '', price: String(product.basePrice ?? product.price), oldPrice: product.oldPrice ? String(product.oldPrice) : '', discountPercent: product.discountPercent ? String(product.discountPercent) : '', discountActive: Boolean(product.discountActive), discountStartsAt: product.discountStartsAt ? String(product.discountStartsAt).slice(0, 16) : '', discountEndsAt: product.discountEndsAt ? String(product.discountEndsAt).slice(0, 16) : '', installmentAvailable: Boolean(product.installmentAvailable), installmentMinMonths: String(product.installmentMinMonths ?? 2), installmentMaxMonths: String(product.installmentMaxMonths ?? 6), stock: String(product.stock), description: product.description, descriptionAr: product.descriptionAr || '', descriptionEn: product.descriptionEn || '', imageUrl: product.image, images: JSON.stringify(product.images || []), specifications: product.specifications.join(', '), specificationsAr: product.specificationsAr?.join(', ') || '', specificationsEn: product.specificationsEn?.join(', ') || '', variants: JSON.stringify(product.variants || []), featured: Boolean(product.isBestSeller), active: product.active !== false };
}
...
function ProductsManagement({ products, categories, onRefresh }: { products: ApiProduct[]; categories: string[]; onRefresh: () => Promise<void> }) {
  ...
  const [installmentFilter, setInstallmentFilter] = useState<'all' | 'enabled' | 'disabled'>('all');
  const visibleProducts = products.filter((product) => installmentFilter === 'all' || (installmentFilter === 'enabled' ? Boolean(product.installmentAvailable) : !product.installmentAvailable));
  const submit = async (event: React.FormEvent) => {
    event.preventDefault(); setError(''); if (!form) return;
    try {
      const variants = JSON.parse(form.variants || '[]');
      const payload = { name: form.nameEn || form.nameAr || form.name, nameAr: form.nameAr || null, nameEn: form.nameEn || null, slug: form.slug || form.nameEn || form.nameAr || form.name, category: form.category, categoryAr: form.categoryAr || null, categoryEn: form.categoryEn || null, price: Number(form.price), oldPrice: form.oldPrice ? Number(form.oldPrice) : null, discountPercent: form.discountPercent ? Number(form.discountPercent) : 0, discountActive: form.discountActive, discountStartsAt: form.discountStartsAt || null, discountEndsAt: form.discountEndsAt || null, installmentAvailable: form.installmentAvailable, installmentMinMonths: form.installmentAvailable ? Number(form.installmentMinMonths) : null, installmentMaxMonths: form.installmentAvailable ? Number(form.installmentMaxMonths) : null, stock: Number(form.stock), description: form.description, descriptionAr: form.descriptionAr || null, descriptionEn: form.descriptionEn || null, imageUrl: form.imageUrl || parseImageList(form.images)[0] || '', images: parseImageList(form.images), specifications: form.specifications.split(',').map((value) => value.trim()).filter(Boolean), specificationsAr: form.specificationsAr ? form.specificationsAr.split(',').map((value) => value.trim()).filter(Boolean) : null, specificationsEn: form.specificationsEn ? form.specificationsEn.split(',').map((value) => value.trim()).filter(Boolean) : null, variants, featured: form.featured, active: form.active };
  ...
  return <div className="space-y-6"><div className="flex flex-col justify-between gap-4 sm:flex-row sm:items-end"><div><p className="mb-2 text-xs font-bold uppercase tracking-[.18em] text-primary">المنتجات والمخزون</p><h2 className="font-display text-5xl tracking-[-.05em]">المنتجات</h2></div><button onClick={() => setForm(emptyForm)} className="flex items-center justify-center gap-2 rounded-xl bg-primary px-4 py-3 text-sm font-bold text-primary-foreground"><Plus className="h-4 w-4" /> إضافة منتج</button></div>{form && <ProductForm form={form} setForm={setForm} submit={submit} error={error} categories={categories} />}
    <div className="flex flex-col gap-3 rounded-2xl border border-border bg-card p-4 shadow-soft sm:flex-row sm:items-center sm:justify-between"><div><p className="text-sm font-bold">حالة التقسيط</p><p className="mt-1 text-xs text-muted-foreground">اعرف بسرعة المنتجات المتاح لها التقسيط.</p></div><select value={installmentFilter} onChange={(event) => setInstallmentFilter(event.target.value as 'all' | 'enabled' | 'disabled')} className="w-full rounded-xl border border-border bg-background px-4 py-3 text-sm font-semibold outline-none focus:ring-2 focus:ring-primary sm:w-60"><option value="all">كل المنتجات</option><option value="enabled">متاح لها تقسيط</option><option value="disabled">بدون تقسيط</option></select></div>
    <div className="overflow-x-auto rounded-2xl border border-border bg-card shadow-soft"><table className="w-full min-w-[760px]"><thead><tr className="border-b border-border text-xs uppercase tracking-wider text-muted-foreground"><th className="px-5 py-4">المنتج</th><th className="px-5 py-4">القسم</th><th className="px-5 py-4">السعر</th><th className="px-5 py-4">المخزون</th><th className="px-5 py-4">التقسيط</th><th className="px-5 py-4">الحالة</th><th className="px-5 py-4">الإجراءات</th></tr></thead><tbody>{visibleProducts.map((product) => <tr key={product.backendId || product.id} className="border-b border-border last:border-0"><td className="px-5 py-4"><div className="flex items-center gap-3"><img src={product.image} alt="" className="h-12 w-12 rounded-xl object-cover" /><div><p className="font-semibold">{product.name}</p><p className="text-xs text-muted-foreground">{product.id}</p></div></div></td><td className="px-5 py-4 text-sm text-muted-foreground">{product.category}</td><td className="px-5 py-4 text-sm font-bold">{money(product.price)}</td><td className="px-5 py-4"><div className="flex items-center gap-2"><button onClick={() => saveStock(product, product.stock - 1)} className="h-8 w-8 rounded-lg border border-border hover:bg-muted">−</button><span className="w-8 text-center text-sm font-bold">{product.stock}</span><button onClick={() => saveStock(product, product.stock + 1)} className="h-8 w-8 rounded-lg border border-border hover:bg-muted">+</button></div></td><td className="px-5 py-4">{product.installmentAvailable ? <span className="inline-flex rounded-full bg-green-100 px-3 py-1 text-xs font-bold text-green-800">متاح</span> : <span className="inline-flex rounded-full bg-muted px-3 py-1 text-xs font-bold text-muted-foreground">غير متاح</span>}</td><td className="px-5 py-4"><button onClick={() => toggleActive(product)} className={`rounded-full px-3 py-1 text-xs font-bold ${product.active === false ? 'bg-muted text-muted-foreground' : product.stock <= 5 ? 'bg-yellow-100 text-yellow-800' : 'bg-green-100 text-green-800'}`}>{product.active === false ? 'غير مفعل' : product.stock <= 5 ? 'مخزون منخفض' : 'مفعل'}</button></td><td className="px-5 py-4"><div className="flex gap-3 text-sm font-semibold"><button onClick={() => setForm(toProductForm(product))} className="text-primary hover:underline">تعديل</button><button onClick={() => remove(product)} className="text-destructive hover:underline">تعطيل</button></div></td></tr>)}</tbody></table>{!visibleProducts.length && <p className="p-10 text-center text-sm text-muted-foreground">{products.length ? 'لا توجد منتجات مطابقة لهذا الفلتر.' : 'لا توجد منتجات. أضف أول منتج الآن.'}</p>}</div></div>;
}
