...
function MobileMenu({ close }: { close: () => void }) {
  ...
  return <div className="absolute inset-x-0 top-[74px] max-h-[calc(100dvh-120px)] overflow-y-auto border-b border-border bg-[#f4ecdf] p-5 shadow-float lg:hidden animate-rise">
...
function ReceiptPage() {
  ...
  const paymentLabel = order.customer.paymentMethod === 'installment' ? (isArabic ? 'تقسيط' : 'Installment') : order.customer.paymentMethod === 'cod' ? (isArabic ? 'الدفع عند الاستلام' : 'Cash on delivery') : (isArabic ? 'تحويل بنكي / إنستاباي' : 'Bank transfer / InstaPay');
  ...
  const displayOrderNumber = String(order.id).padStart(3, '0');
  const labelClass = 'text-[11px] font-bold uppercase tracking-[.14em] text-muted-foreground';
  return <div dir={isArabic ? 'rtl' : 'ltr'} className="min-h-screen bg-[#f4ecdf] px-4 py-8 sm:px-6 lg:px-8 lg:py-14">
    <div className="mx-auto max-w-4xl">
      <div className="mb-8 flex flex-col items-center text-center">
        <div className="mb-5 flex h-16 w-16 items-center justify-center rounded-full bg-[#d7efdf] text-[#2E9B68]"><Check size={32} strokeWidth={2.5} /></div>
        <p className="mb-3 text-xs font-bold uppercase tracking-[.22em] text-primary">{isArabic ? 'تم تأكيد طلبك' : 'Order confirmed'}</p>
        <h1 className="font-display text-4xl leading-none tracking-[-.05em] text-[#3D2A1E] sm:text-6xl">{isArabic ? 'إيصال طلبك' : 'Your order receipt'}</h1>
        <p className="mt-4 max-w-xl text-sm leading-7 text-[#765e4c]">{isArabic ? 'تم استلام طلبك بنجاح. أرسلنا لك كل التفاصيل في رسالة نصية جاهزة عبر واتساب.' : 'Your order was received successfully. All details are ready to send as a text message through WhatsApp.'}</p>
      </div>
      <article className="overflow-hidden rounded-[2rem] border border-[#dfccb2] bg-[#fffaf4] shadow-[0_22px_70px_rgba(61,42,30,.12)]">
        <div className="flex flex-col gap-6 bg-[#3D2A1E] p-6 text-[#f4ecdf] sm:flex-row sm:items-end sm:justify-between sm:p-9">
          <div><div className="flex items-center gap-3"><img src={storeSettings.logoUrl || logo} alt="" className="h-11 w-11 rounded-full object-cover" /><span className="font-display text-3xl">{storeSettings.storeName}</span></div><p className="mt-4 text-sm text-[#cdbbab]">{isArabic ? 'تفاصيل الطلب' : 'Order details'}</p></div>
          <div className="sm:text-end"><p className="text-[10px] font-bold uppercase tracking-[.18em] text-[#d9a77d]">{isArabic ? 'رقم الطلب' : 'Order number'}</p><p className="mt-1 font-display text-2xl">{displayOrderNumber}</p></div>
        </div>
        <div className="grid gap-5 border-b border-[#eadccb] p-6 sm:grid-cols-2 sm:p-9">
          <div><p className={labelClass}>{isArabic ? 'الاسم' : 'Name'}</p><p className="mt-2 font-semibold text-[#3D2A1E]">{order.customer.name}</p></div>
          <div><p className={labelClass}>{isArabic ? 'التاريخ' : 'Date'}</p><p className="mt-2 font-semibold text-[#3D2A1E]">{date.toLocaleString(isArabic ? 'ar-EG' : 'en-KE', { dateStyle: 'medium', timeStyle: 'short' })}</p></div>
          <div><p className={labelClass}>{isArabic ? 'رقم الهاتف' : 'Phone'}</p><p dir="ltr" className="mt-2 text-start font-semibold text-[#3D2A1E]">{order.customer.phone}</p></div>
          <div><p className={labelClass}>{isArabic ? 'طريقة الدفع' : 'Payment method'}</p><p className="mt-2 font-semibold text-[#3D2A1E]">{paymentLabel}</p></div>
          {order.customer.paymentMethod === 'installment' && <><div><p className={labelClass}>{isArabic ? 'رقم الهوية' : 'ID number'}</p><p className="mt-2 font-semibold text-[#3D2A1E]">{order.customer.idNumber || '—'}</p></div><div><p className={labelClass}>{isArabic ? 'يوم السداد الشهري' : 'Monthly payment day'}</p><p className="mt-2 font-semibold text-[#3D2A1E]">{order.installmentPaymentDay || '—'}</p></div></>}
          <div className="sm:col-span-2"><p className={labelClass}>{isArabic ? 'الموقع والعنوان' : 'Location and address'}</p><p className="mt-2 leading-7 text-[#3D2A1E]">{order.customer.governorate}، {order.customer.city}، {order.customer.address}</p></div>
        </div>
        <div className="p-6 sm:p-9"><div className="flex items-end justify-between gap-4"><div><p className={labelClass}>{isArabic ? 'محتويات الطلب' : 'Order items'}</p><h2 className="mt-2 font-display text-3xl text-[#3D2A1E]">{isArabic ? 'اختياراتك' : 'Your selection'}</h2></div><span className="rounded-full bg-[#d7efdf] px-3 py-1 text-xs font-bold text-[#26754d]">{isArabic ? 'قيد المراجعة' : 'Pending confirmation'}</span></div>
          <div className="mt-6 divide-y divide-[#eadccb]">{order.items.map((item) => { const product = products.find((entry) => entry.id === item.id); if (!product) return null; const visible = localizedProduct(product, language); return <div key={item.id} className="flex items-center justify-between gap-4 py-4"><div><p className="font-semibold text-[#3D2A1E]">{visible.name}</p><p className="mt-1 text-sm text-[#765e4c]">{isArabic ? 'الكمية' : 'Quantity'}: {item.quantity}</p></div><p className="font-bold text-primary">{money(product.price * item.quantity, language)}</p></div>; })}</div>
          <div className="mt-6 rounded-2xl bg-[#f4ecdf] p-5"><div className="ml-auto max-w-sm space-y-3 text-sm"><div className="flex justify-between"><span className="text-[#765e4c]">{isArabic ? 'المجموع الفرعي' : 'Subtotal'}</span><span>{money(order.subtotal, language)}</span></div><div className="flex justify-between"><span className="text-[#765e4c]">{isArabic ? 'الشحن' : 'Shipping'}</span><span>{order.shipping ? money(order.shipping, language) : (isArabic ? 'مجاني' : 'Free')}</span></div><div className="flex justify-between border-t border-[#d1b99c] pt-3 text-xl font-bold text-[#3D2A1E]"><span>{isArabic ? 'الإجمالي' : 'Total'}</span><span>{money(order.total, language)}</span></div>{order.customer.paymentMethod === 'installment' && <div className="border-t border-[#d1b99c] pt-3 text-[#765e4c]"><div className="flex justify-between"><span>{isArabic ? 'نظام التقسيط' : 'Installment plan'}</span><span className="font-bold text-[#3D2A1E]">{planName}</span></div><div className="mt-2 flex justify-between"><span>{isArabic ? 'القسط الشهري' : 'Monthly payment'}</span><span className="font-bold text-primary">{order.installmentMonthlyPayment ? money(order.installmentMonthlyPayment, language) : '—'}</span></div></div>}</div></div>
        </div>
      </article>
      <div className="mt-7 flex flex-col justify-center gap-3 sm:flex-row"><a href={whatsappUrl(whatsappMessage)} target="_blank" rel="noreferrer" className="flex h-12 items-center justify-center gap-2 rounded-xl bg-[#2E9B68] px-6 text-sm font-bold text-white shadow-sm transition-transform hover:-translate-y-0.5" data-testid="link-send-receipt-whatsapp"><MessageCircle size={18} /> {isArabic ? 'إرسال الإيصال عبر واتساب' : 'Send receipt via WhatsApp'}</a><button onClick={() => window.print()} className="flex h-12 items-center justify-center gap-2 rounded-xl border border-[#d1b99c] bg-[#fffaf4] px-6 text-sm font-bold text-[#3D2A1E] transition-colors hover:bg-white" data-testid="button-print-receipt"><Printer size={18} /> {isArabic ? 'طباعة الإيصال' : 'Print receipt'}</button><Link href="/products" className="flex h-12 items-center justify-center gap-2 rounded-xl px-5 text-sm font-bold text-primary">{isArabic ? 'متابعة التسوق' : 'Continue shopping'} <ArrowRight size={17} /></Link></div>
    </div>
  </div>;
}
