...
async function ensureDefaultCategories() {
  ...
  await db.insert(categoriesTable as any).values(DEFAULT_CATEGORIES.map((name) => ({ name, slug: slugify(name), active: true })));
}
...
router.post("/", requireAdmin, async (req, res) => {
  try {
    ...
    const category = ((await db.insert(categoriesTable as any).values({ name, nameAr: nameAr || null, nameEn: nameEn || null, slug: slugify(String(req.body?.slug || nameEn || name)), active: req.body?.active !== false }).returning()) as any[])[0];
...
router.put("/:id", requireAdmin, async (req, res) => {
  try {
    ...
    const [category] = await db.update(categoriesTable as any).set({ name, nameAr: nameAr || null, nameEn: nameEn || null, slug: slugify(String(req.body?.slug || nameEn || name)), active: req.body?.active !== false, updatedAt: new Date() }).where(eq(categoriesTable.id, id)).returning();
